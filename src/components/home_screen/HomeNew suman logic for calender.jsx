...
function HomeNew() {
  ...
  // console.log('product', allRentalProduct);
  console.log('allRentalProduct', JSON.stringify(allRentalProduct, null, 2));
  ...
  console.log('allorderproduct', JSON.stringify(allorderproduct, null, 2));
  ...
  const [diabledDate, setDisabledDate] = useState({});
  const getMarkedDates = () => {
    ...
    // Loop until the date is before today
    ...
    // Merge disabledDates with the markedDates (for startDate, endDate, and range marking)
  ...
  const orders = allorderproduct.flatMap(item => item.productdata);
  ...
  function calculateAvailableQuantity(product, orderValue, startDateValue) {
    ...
    const ordersForDate = orderValue.filter(order => {
      return (
        moment(order.ordered_date).format('YYYY-MM-DD') === formattedStartDate
        // && order.id === product?._id
      );
    });

    console.log('Orders for the selected date:', ordersForDate);

    // Calculate the total applied quantity from the filtered orders
